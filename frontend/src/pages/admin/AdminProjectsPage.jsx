...
import { useAllProjects, useProjectMembers, useAllEmployees } from "../../hooks/useTasks";
...
const CreateProjectModal = ({ employees, onClose, onCreate }) => {
  const [form, setForm] = useState({ title:"", description:"", managerId:"", priority:"medium", startDate:"", endDate:"" });
  const [saving, setSaving] = useState(false);
  const [err,    setErr]    = useState("");
  const h = (e) => setForm(p => ({...p,[e.target.name]:e.target.value}));
  const submit = async () => {
    if (!form.title||!form.managerId||!form.startDate||!form.endDate) { setErr("Title, manager, start date and end date are required."); return; }
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg">
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-bold text-slate-800">Create Project</h2>
          </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div><label className="text-xs font-semibold text-slate-500 mb-1.5 block">Project Title *</label>
            <input name="title" value={form.title} onChange={h} placeholder="e.g. Q3 Infrastructure" className="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400"/></div>
          <div><label className="text-xs font-semibold text-slate-500 mb-1.5 block">Description</label>
            <textarea name="description" value={form.description} onChange={h} rows={2} placeholder="Brief overview…" className="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 resize-none"/></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs font-semibold text-slate-500 mb-1.5 block">Project Manager *</label>
              <select name="managerId" value={form.managerId} onChange={h} className="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400">
            ...
            <div><label className="text-xs font-semibold text-slate-500 mb-1.5 block">Priority</label>
              <select name="priority" value={form.priority} onChange={h} className="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400">
                <option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option>
              </select></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs font-semibold text-slate-500 mb-1.5 block">Start Date *</label>
              <input type="date" name="startDate" value={form.startDate} onChange={h} className="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400"/></div>
            <div><label className="text-xs font-semibold text-slate-500 mb-1.5 block">End Date *</label>
              <input type="date" name="endDate" value={form.endDate} onChange={h} className="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400"/></div>
          </div>
        </div>
        <div className="flex gap-3 p-5 pt-0">
          ...
          <button onClick={submit} disabled={saving} className="flex-1 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 disabled:opacity-60">{saving?"Creating…":"Create Project"}</button>
        </div>
      </div>
    </div>
  );
...
const ProjectCard = ({ project, allEmployees, onStatusChange }) => {
  ...
  const cycleStatus = async () => {
    const next = { active:"on-hold","on-hold":"completed",completed:"active" };
    setStatusSave(true);
    try { await onStatusChange(project._id, { status: next[project.status] }); }
    catch { /* silently ignore */ }
    finally { setStatusSave(false); }
  };
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-5 hover:shadow-md hover:shadow-slate-100 hover:-translate-y-0.5 transition-all">
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {project.description && <p className="text-xs text-slate-400 line-clamp-1 mt-0.5">{project.description}</p>}
            </div>
          </div>
          <button onClick={cycleStatus} disabled={statusSave}
        ...
        <div className="mb-4">
          ...
          <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
            <div className={`h-full rounded-full transition-all ${prog>=75?"bg-emerald-500":prog>=40?"bg-blue-500":"bg-amber-400"}`} style={{width:`${prog}%`}}/>
          </div>
        </div>

        <div className="flex items-center justify-between text-xs text-slate-400">
          <div className="flex items-center gap-1"><Icon name="users" className="w-3.5 h-3.5"/>{project.managerId?.name ?? "No manager"}</div>
          <div className="flex items-center gap-1"><Icon name="calendar" className="w-3.5 h-3.5"/>Ends {end}</div>
        </div>
        ...
        <div className="mt-3 pt-3 border-t border-slate-50 flex gap-2">
          <button onClick={() => setShowTeam(true)}
            ...
            <Icon name="users" className="w-3.5 h-3.5"/>Manage Team
          </button>
        </div>
      </div>
  ...
  return (
    <DashboardLayout title="Project Management">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {loading ? "Loading…" : `${projects.length} total — ${active} active, ${onHold} on hold, ${completed} completed`}
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-5">
        {[
          { label:"Active",    val:active,    color:"bg-emerald-50 border-emerald-200 text-emerald-700" },
          { label:"On Hold",   val:onHold,    color:"bg-amber-50 border-amber-200 text-amber-700" },
          { label:"Completed", val:completed, color:"bg-slate-50 border-slate-200 text-slate-600" },
        ].map(k => (
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        {["","active","on-hold","completed"].map(s => (
          <button key={s} onClick={() => setFilterStatus(s)}
            ...
            {s===""?"All":s==="on-hold"?"On Hold":s.charAt(0).toUpperCase()+s.slice(1)}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-16 text-center">
          ...
          <p className="text-sm text-slate-400 mt-2">Create your first project to get started.</p>
          <button onClick={() => setShowCreate(true)} className="mt-4 px-5 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition-colors">Create Project</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
          {projects.map(p => (
