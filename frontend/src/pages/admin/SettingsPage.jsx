    ...
    <form onSubmit={handlePasswordSubmit}
      ...
      <input
        ...
        className={`w-full border rounded-xl px-3.5 py-2.5 text-sm mb-2 focus:outline-none focus:ring-2 focus:border-blue-400 ${
          pwError ? "border-red-300 focus:ring-red-500/20" : "border-slate-200 focus:ring-blue-500/20"
        }`}
        autoFocus
      />
    ...
    <DashboardLayout title="Settings">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-5 mb-4">
        ...
        {loading ? (
          <div className="space-y-3">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-5 bg-slate-50 rounded-xl animate-pulse" />
            ))}
          </div>
        ) : (
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-5 mb-4">
        ...
        {loading ? (
          ...
          <>
            {/* Code display */}
            <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 flex items-center gap-3 mb-3">
              <p className="text-sm font-bold text-blue-800 font-mono tracking-widest flex-1 break-all select-none">
                {revealed ? (company?.inviteCode ?? "—") : "••••••••••••••••••••••••"}
              </p>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: "Total employees", value: loading ? "…" : employees.length, color: "bg-blue-50 text-blue-700"      },
            { label: "Active",          value: loading ? "…" : active,           color: "bg-emerald-50 text-emerald-700" },
            { label: "Inactive",        value: loading ? "…" : inactive,         color: "bg-slate-50 text-slate-600"    },
          ].map(k => (
