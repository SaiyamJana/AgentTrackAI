...
const EditEmployeeModal = ({ employee, onClose, onSave }) => {
  ...
  const submit = async () => {
    ...
    setErr("");
    try {
      await onSave(employee._id, form);
      onClose();
    } catch (e) {
      setErr(e.message);
    } finally {
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {loading ? (
            [...Array(3)].map((_, i) => (
              <div
                key={i}
                className="h-14 bg-slate-50 rounded-xl animate-pulse mb-2"
              />
            ))
            ...
            <div className="space-y-2">
              {assignments.map(({ project, projectRole }) => (
                ...
                >
                  ...
                  <span
                    className={`text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0 ${roleCfg[projectRole] ?? roleCfg.member}`}
                  >
                    {projectRole}
                  </span>
                </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-5 hover:shadow-md hover:shadow-slate-100 hover:-translate-y-0.5 transition-all">
      <div className="flex items-start justify-between gap-3 mb-4">
        ...
        <span
          className={`text-xs font-semibold px-2.5 py-1 rounded-lg flex items-center gap-1.5 shrink-0 ${s.badge}`}
        >
...
export default function AdminEmployeesPage() {
  ...
  const filtered = employees.filter((e) => {
    ...
    const matchesStatus =
      !filterStatus || (filterStatus === "active" ? e.isActive : !e.isActive);
  ...
  return (
    <DashboardLayout title="Employee Management">
        ...
        {[
          {
            ...
            color: "bg-primary-light border-blue-200 text-primary",
          },
          {
            label: "Active",
            val: activeCount,
      ...
      {/* Search + Filter */}
      <div className="flex flex-col sm:flex-row gap-3 mb-5 page-enter-delay-2">
        <div className="relative flex-1">
          <Icon
            name="users"
            className="w-4 h-4 text-slate-300 absolute left-3.5 top-1/2 -translate-y-1/2"
          />
          <input
            value={search}
      ...
      {loading ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="bg-white rounded-2xl border border-slate-100 h-44 animate-pulse"
            />
          ))}
        </div>
      ) : error ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-16 text-center">
          ...
          <p className="text-sm text-slate-400 mt-2">
            {search || filterStatus
              ...
              : "Employees will appear here once they register with your Secure Code."}
          </p>
        </div>
      ) : (
