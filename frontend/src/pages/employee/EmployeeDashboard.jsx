    ...
    <div className={`bg-white rounded-2xl border p-4 hover:shadow-md hover:shadow-slate-100 hover:-translate-y-0.5 transition-all ${overdue ? "border-red-200" : "border-slate-100"}`}>
      ...
      <div className="mb-3">
        ...
        <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
          <div className={`h-full rounded-full transition-all ${task.status === "completed" ? "bg-emerald-500" : (task.completionPercentage ?? 0) > 50 ? "bg-primary-light0" : "bg-amber-400"}`}
      ...
      <div className="flex items-center justify-between">
        ...
        {task.status !== "completed" ? (
          <button onClick={() => onUpdate(task)} className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-primary-light text-primary hover:bg-primary-light transition-colors">Update</button>
        ) : (
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        ...
        <div className="p-5 space-y-5">
          ...
          <div>
            <div className="flex justify-between mb-2"><label className="text-xs font-semibold text-slate-500">Completion</label><span className="text-sm font-bold text-primary">{pct}%</span></div>
            ...
            <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden mt-2">
              <div className={`h-full rounded-full transition-all ${pct===100?"bg-emerald-500":pct>50?"bg-primary-light0":"bg-amber-500"}`} style={{width:`${pct}%`}}/>
            </div>
          </div>
          <div>
            ...
            <div className="flex gap-2">
              {["pending","in-progress","completed"].map(s => (
                <button key={s} onClick={() => setStatus(s)} className={`flex-1 py-2 rounded-xl text-xs font-semibold border transition-all ${status===s?"border-blue-400 bg-primary-light text-primary":"border-slate-200 text-slate-500 hover:border-slate-300"}`}>
          ...
          <div>
            ...
            <input type="number" value={hours} onChange={e => setHours(Number(e.target.value))} min={0} className="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-blue-400 transition-all"/>
          </div>
        </div>
        <div className="flex gap-3 p-5 pt-0">
          ...
          <button onClick={submit} disabled={saving} className="flex-1 py-2.5 rounded-xl bg-primary text-white text-sm font-semibold hover:bg-primary-hover transition-colors disabled:opacity-60">{saving?"Saving…":"Save Progress"}</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 page-enter-delay-2">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <a href="/employee/tasks" className="text-xs text-primary font-semibold hover:text-primary-hover">View all →</a>
          </div>
          {tasksLoading ? (
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <a href="/employee/projects" className="text-xs text-primary font-semibold hover:text-primary-hover">View all →</a>
            </div>
            {projLoading ? (
          ...
          {tasks.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-100 p-5">
              ...
              {[
                ...
                { label: "In Progress", val: stats.inProgress, total: stats.total, color: "bg-primary-light0" },
