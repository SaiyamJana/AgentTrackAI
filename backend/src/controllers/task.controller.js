...
export const createTask = asyncHandler(async (req, res) => {
  ...
  if (req.user.role !== "admin") {
    ...
    if (!manager) throw new ApiError(403, "Only project manager or admin can create tasks");
  }
  ...
  if (subManagerAssignment.employeeId.toString() === project.managerId.toString())
    throw new ApiError(400, "Project manager cannot be task sub-manager");
  ...
  // Create TaskAssignment for sub-manager with equal-split contribution (100% initially — only 1 member)
  await TaskAssignment.create({
    taskId: task._id,
    employeeId: subManagerId,
    assignedBy: req.user._id,
    contributionPercentage: null, // equal split (100/1 = 100%)
    estimatedPersonalHours: computePersonalHours(estimatedHours, 100, 1),
  });
...
export const removeTaskMembers = asyncHandler(async (req, res) => {
  ...
  task.teamMembers = task.teamMembers.filter(member => member.toString() !== employeeId.toString());
  await task.save();
